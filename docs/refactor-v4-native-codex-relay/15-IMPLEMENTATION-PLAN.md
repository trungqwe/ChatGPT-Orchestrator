...
| V4-09 | One-shot full cycle | IMPLEMENTATION_COMPLETE / EXTERNAL_REVIEW_PENDING (WP-V4-09A design APPROVED_CLOSED; one-shot coordinator implemented over existing Registry/workspace/broker/Codex authorities with exact bound-thread resume, pinned model policy, Registry Gates A/B/C/D/final, S0/S1/S2/S3 freshness, max one fresh owned worker dispatch, idempotent replay rejection, same-thread Turn B review, bounded cleanup precedence, no automatic resend; bounded auditSubjectId; production-compatible prompt validation; finite turn timeout; broker-bounded worker wait timeout; bounded propagated result codes; OSC 105/105 plus full 18 deterministic suites pass; real workspace acceptance remains pending WP-V4-09C) |
