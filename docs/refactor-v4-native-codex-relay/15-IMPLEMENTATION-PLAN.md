...
| V4-09 | One-shot full cycle | DESIGN_COMPLETE / EXTERNAL_REVIEW_PENDING (WO-V4-09A-R2: canonicalizeProjectRoot runtime authority at every Registry gate; policy.max_active_dispatches===1 required at Gate A; exact awaitAuditDecisionV1 error boundary — AUDIT_DECISION_ITEMS_INCOMPLETE→DECISION_INVALID, AUDIT_DECISION_TURN_NOT_COMPLETED→AUDITOR_TURN_FAILED, raw completion timeout→AUDITOR_TURN_UNCERTAIN; close failure overrides APPROVED; 14 invariants for full-cycle pass; OSC >= 42; 18 deterministic suites expected) |
