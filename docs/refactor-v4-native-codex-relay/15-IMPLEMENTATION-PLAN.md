...
| V4-09 | One-shot full cycle | DESIGN_COMPLETE / EXTERNAL_REVIEW_PENDING (WO-V4-09A-R3: execution authority > cleanup authority; primary uncertainty cannot be masked by auditor close failure; cleanup status recorded separately; broker idempotent replay cannot be adopted as one-shot-owned dispatch; only fresh DISPATCH_ACCEPTED result permits wait; close failure overrides APPROVED; 14 invariants for full-cycle pass; OSC >= 42; 18 deterministic suites expected) |
