...
| V4-09 | One-shot full cycle | DESIGN_COMPLETE / EXTERNAL_REVIEW_PENDING (WO-V4-09A-R1: exact source APIs corrected — workspacePort.getWorkspaceState, broker.getWorkerStatus/dispatchWorker/waitWorker, awaitAuditDecisionV1, resolveAuditorModelPolicy({policy,models}); explicit reviewPrompt; single auditSubjectId across both turns; fresh Registry Gates A/B/C/D/final; worker IDLE pre-turn gate; worker model policy consistency gate; auditor request uncertainty preserved as AUDITOR_TURN_UNCERTAIN; adapter close in finally boundary; OSC >= 42; 18 deterministic suites expected) |
