...
| V4-09 | One-shot full cycle | IN_PROGRESS (WP09A APPROVED_CLOSED; WP09B APPROVED_CLOSED; WP09C D1 delivery-ack design APPROVED_CLOSED; delivery-ack implementation completed locally: AO exit 0 alone no longer yields DISPATCH_ACCEPTED; exact authoritative transcript boundary required; missing boundary after successful AO command becomes non-definitive DISPATCH_UNCERTAIN; wait missing a previously proven boundary becomes PROVENANCE_AMBIGUOUS; deterministic worker tests: WA55 + ACK12 + ACK-INT1 (68/68 PASS); full deterministic suite: 18 suites pass; external review pending; existing real P1 dispatch still unreconciled; real re-acceptance not authorized; WP10 NOT_STARTED) |
