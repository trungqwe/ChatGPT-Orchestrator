...
- **No Prose Authority**: Relay tuyệt đối không trích xuất quyền điều phối (lifecycle authority) từ văn bản tự do (prose), regex, khối markdown fences (` ```json `), hoặc heurstic keyword matching. Quyết định chỉ có hiệu lực khi đi qua quy trình validation có cấu trúc `AuditDecisionV1`.
...
- **Bounded Error Diagnostics**: Thông báo lỗi và chẩn đoán không bao giờ in toàn bộ chuỗi raw text, directive lớn hay JSON dị dạng nhằm tránh rò rỉ prompt injection payload hoặc làm quá tải logging.
