...
- `startThread`: Bắt buộc `cwd` tuyệt đối (`path.isAbsolute`). Cấu hình bảo mật provider ổn định: `sandbox: "readOnly"` và `approvalPolicy: "never"`. Từ chối các invented boolean protocol fields (`readOnly`, `workspaceWrite`, `dangerFullAccess`) bằng `CODEX_APP_SERVER_SECURITY_VIOLATION`. Bảo toàn opaque `thread.id` và `thread.sessionId` byte-for-byte. Không ghi vào Registry. Loại bỏ hoàn toàn việc forward các param test hook có tiền tố `_`.
...
- `startReview`: Chỉ cho phép `delivery = "inline"` trong v4 MVP (từ chối `detached`). Target bắt buộc phải là structured object (tối thiểu `{ type: "uncommittedChanges" }`, từ chối plain string với `INVALID_REVIEW_TARGET`). Yêu cầu `reviewThreadId === threadId`, từ chối `CODEX_APP_SERVER_THREAD_MISMATCH`.
