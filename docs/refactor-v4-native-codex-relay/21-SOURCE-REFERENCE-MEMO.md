...
Kiểm chứng dựa trên schema sinh ra từ chính bản cài đặt Codex:
- `ThreadStartParams`: Chứa các trường `{ cwd, approvalPolicy: "never", sandbox: "readOnly" }`.
...
2. Our Adapter Policy (WP-V4-03A / WO-V4-03AF):
   ...
   - Bắt buộc `cwd` tuyệt đối khi `thread/start` và cấu hình `sandbox: "readOnly"`, `approvalPolicy: "never"` (từ chối `dangerFullAccess`, `workspaceWrite`, `readOnly`).
...
3. Status & Future V4 Behavior:
   - WP-V4-03A: PENDING_EXTERNAL_REVIEW (đã hoàn thành WO-V4-03AF Corrective Seal, chờ phê duyệt).
...
Persistent project thread, App Server control plane, read-only auditor policy và logical model tiers là quyết định V4. WP-V4-03A đã niêm phong conformance theo đúng stable protocol qua WO-V4-03AF; WP-V4-03B sẽ nghiệm thu real App Server khi có quyết định bắt đầu.
